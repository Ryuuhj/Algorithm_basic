import java.util.PriorityQueue;

import 그래프의_표현.P1325_효율적인해킹;
import 그래프의_표현.P1707_이분그래프;
import 그래프의_표현.P18352_특정거리의_도시찾기;
import 그래프의_표현.P2251_물통;
import 다익스트라.P1753_최단경로;
import 다익스트라.P1854_K번째_최단경로_찾기;
import 다익스트라.P1916_최소비용_구하기;
import 벨만_포드.P11657_타임머신;
import 벨만_포드.P1219_오민식의_고민;
import 위상정렬.P1516_게임개발;
import 위상정렬.P1948_임계경로;
import 위상정렬.P2252_줄세우기;
import 유니온_파인드.P1043_거짓말;
import 유니온_파인드.P1717_집합의표현;
import 유니온_파인드.P1976_여행가자;
import 플로이드_워셜.P11403_경로찾기;
import 플로이드_워셜.P11404_플로이드;

public class App {

    //static P18352_특정거리의_도시찾기 a = new P18352_특정거리의_도시찾기();
    //static P1325_효율적인해킹 a= new P1325_효율적인해킹();
    // static P1707_이분그래프 a = new P1707_이분그래프();
    // static P2251_물통 a = new P2251_물통();
    //static P1717_집합의표현 a = new P1717_집합의표현();
    // static P1976_여행가자 a = new P1976_여행가자();
    // static P1043_거짓말 a = new P1043_거짓말();
    //static P2252_줄세우기 a = new P2252_줄세우기();
    // static P1516_게임개발 a = new P1516_게임개발();
    // static P1948_임계경로 a = new P1948_임계경로();
    // static P1753_최단경로 a = new P1753_최단경로();
    // static P1916_최소비용_구하기 a = new P1916_최소비용_구하기();
    //static P1854_K번째_최단경로_찾기 a = new P1854_K번째_최단경로_찾기();
    // static P11657_타임머신 a = new P11657_타임머신();
    // static P1219_오민식의_고민 a = new P1219_오민식의_고민();
//    static P11404_플로이드 a = new P11404_플로이드();
    static P11403_경로찾기 a = new P11403_경로찾기();
    /*public static void main(String[] args) throws Exception {
        a.out();
    }*/
}
